...
for PARALLEL_SETTING in [True, False]:
    ...
    for QUERY_HARDNESS, OOD, HIGH_DIM in [(None, None, True), (None, None, False)]:
        ...
        if PARALLEL_SETTING:
            data_dir = "./fig/max_rec_tree_parallel"
            out_path = f"./fig/max_rec_tree_parallel/summary{postfix}.csv"
        else:
            data_dir = "./fig/max_rec_tree_sequential"
            out_path = f"./fig/max_rec_tree_sequential/summary{postfix}.csv"
        ...
        for fname in os.listdir(data_dir):
            m_fname = fname.replace("_20_variants", "")
            m = pattern.match(m_fname)
            if not m:
                continue
        ...
        for constraint in constraints:
            ...
            # 只保留两个 category 都 >= 0 的 dataset
            valid_datasets = [
                v for v in datasets_for_constraint.values()
                if v["standalone"] >= 0 and v["pairing"] >= 0
            ]

            for cat in categories:
                recalls = [v[cat] for v in valid_datasets]
            ...
            for ds, vals in datasets_for_constraint.items():
                ...
                if s > p:
                    standalone_win += 1
                    standalone_win_ds = standalone_win_ds + ds + ", "
                elif p > s:
                    pairing_win += 1
                    paring_win_ds = paring_win_ds + ds + ", "
                # tie 的情况不加分
            ...
            results["win_ds"].append(standalone_win_ds)
            results["win_ds"].append(paring_win_ds)
