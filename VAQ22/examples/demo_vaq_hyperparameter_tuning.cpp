...
int main(int argc, char **argv) {
  ...
  int best_min_bit, best_max_bit; 
  {
    ...
    for (minBit=1;minBit<=7;minBit++) {
      precision=evaluateParams(best_alg,minBit, best_max_bit, reduction_threshold, -1.0f, kmeans_sub_space);
      std::cout<<"algo: "<<best_alg<<" reduction_threshold: "<<reduction_threshold<<" minBit: "<<minBit<<" maxBit: "<<best_max_bit<<" kmeans_sub_space: "<<kmeans_sub_space<<" Precision: "<<precision<<endl;
      if (precision>bestPrecision) {
        best_min_bit=minBit;
        // best_max_bit=maxBit;
        bestPrecision=precision;
    ...
    for (maxBit=8;maxBit<=13;maxBit++) {
      precision=evaluateParams(best_alg,best_min_bit, maxBit, reduction_threshold, -1.0f, kmeans_sub_space);
      std::cout<<"algo: "<<best_alg<<" reduction_threshold: "<<reduction_threshold<<" minBit: "<<best_min_bit<<" maxBit: "<<maxBit<<" kmeans_sub_space: "<<kmeans_sub_space<<" Precision: "<<precision<<endl;
      if (precision>bestPrecision) {
        // best_min_bit=minBit;
        best_max_bit=maxBit;
        bestPrecision=precision;
