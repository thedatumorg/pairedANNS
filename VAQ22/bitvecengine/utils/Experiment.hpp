...
double getAvgRecall(const std::vector<int> &labels, const std::vector<std::vector<int>> &topnn, const int K, const int _K) {
  double ans = 0.0;

  int nq = labels.size() / _K;
  
  for (int q_idx=0; q_idx<nq; q_idx++) {
    int ct = 0;
    for (int k_idx=0; k_idx<K; k_idx++) {
      for (int j=0; j<K; j++) {
        if (labels[q_idx * _K + k_idx] == topnn[q_idx][j]-IdxOffset) {
          ct++;
          break;
        }
      }
...
template <int IdxOffset=0>
double getAvgRecallDistance(const std::vector<float> &labels, const std::vector<std::vector<float>> &topnn, const int K) {
  double ans = 0.0;
  ...
  for (int q_idx=0; q_idx<nq; q_idx++) {
    ...
    for (int k_idx=0; k_idx<K; k_idx++) {
      for (int j=0; j<K; j++) {
        if (labels[q_idx * K + k_idx] <= topnn[q_idx][j]) {
          // std::cout<<labels[q_idx * K + k_idx] <<" "<< topnn[q_idx][j];
          ct++;
          break;
        }
      }
