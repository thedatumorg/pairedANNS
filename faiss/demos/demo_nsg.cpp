...
int main(int argc, char* argv[]) {
    ...
    int K = std::stoi(argv[5]);            // NSG out-degree
    ...
    index.nsg.build_L = build_L;
    index.nsg.out_degree = K;

    auto start_training = std::chrono::high_resolution_clock::now();
    index.train(nt, xt);   // NSG requires a training step (builds base graph)
    index.add(nt, xt);     // add base vectors
    index.build(nq);       // refine the graph (optional but improves recall)
